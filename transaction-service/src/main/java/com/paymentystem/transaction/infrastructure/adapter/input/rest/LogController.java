...
public class LogController {
    ...
    private static final String LOG_PATH = "logs/transaction-service";
    ...
    public ResponseEntity<Resource> downloadByDate(
        ...
        String fileName = "transaction-service." + date.format(DateTimeFormatter.ISO_DATE) + ".log";
        File logFile = new File(LOG_PATH + "/" + fileName);

        // Si es el día de hoy, sirve el archivo activo
        if (date.equals(LocalDate.now())) {
            logFile = new File(LOG_PATH + "/transaction-service.log");
            fileName = "transaction-service-" + LocalDate.now() + ".log";
        }
    ...
    @GetMapping("/list")
    // s@PreAuthorize("hasAuthority('SCOPE_ADMIN') or hasRole('ADMIN')")
