...
public class LogController {
    ...
    private static final String LOG_PATH = "logs/account-service";
    ...
    public ResponseEntity<Resource> downloadByDate(
        ...
        String fileName = "account-service." + date.format(DateTimeFormatter.ISO_DATE) + ".log";
        File logFile = new File(LOG_PATH + "/" + fileName);

        // Si es el día de hoy, sirve el archivo activo
        if (date.equals(LocalDate.now())) {
            logFile = new File(LOG_PATH + "/account-service.log");
            fileName = "account-service-" + LocalDate.now() + ".log";
        }
